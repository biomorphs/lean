...
function MakeSDFEntity(pos,bmin,bmax,res,drawShader,sdfShader, mat, octreedepth)
	...
	sdfModel:SetMaterialEntity(mat)
	sdfModel:SetOctreeDepth(octreedepth or 4)
end
...
function Planet.Init()
	...
	local p = {0,0,0}
	MakeSDFEntity({0,0,0},p,{p[1]+blockSize[1],p[2]+blockSize[2],p[3]+blockSize[3]},res, DrawPlanetShader, SDFPlanetShader, MakePlanetMaterial())
	MakeSDFEntity({0,0,0},p,{p[1]+blockSize[1],p[2]+blockSize[2],p[3]+blockSize[3]},res, DrawWaterShader, SDFWaterShader, MakeWaterMaterial(), 2)
end
