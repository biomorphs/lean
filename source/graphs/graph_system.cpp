...
bool GraphSystem::Initialise()
{
	...
	g_testGraph.AddOutputPin(1, "RunNext", "Execution");
	g_testGraph.AddOutputPin(2, "Result", "int");
	...
	uint16_t printIntId = g_testGraph.AddNode(new PrintIntNode());

	g_testGraph.AddConnection(Graphs::Node::InvalidID, 0, addIntId, 0);		// graph execute -> addInt execute
	...
	g_testGraph.AddConnection(addIntId, 4, printIntId, 1);					// addInt result -> print value
	g_testGraph.AddConnection(addIntId, 4, Graphs::Node::InvalidID, 2);		// addInt result -> graph result

	g_testGraph.AddConnection(addIntId, 3, printIntId, 0);					// addInt exec -> print exec

	g_testGraph.AddConnection(printIntId, 2, Graphs::Node::InvalidID, 1);	// print exec -> graph out exec
	...
	SDE_PROF_EVENT();
	{
		...
		while (graphRunning)
		{
			...
			if (stepResult == Graphs::GraphContext::Completed)
			{
				...
				int resultValue = runContext.ReadGraphPin(Graphs::Node::InvalidID, 2);
